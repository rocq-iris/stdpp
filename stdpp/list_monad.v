...
(** New section to allow lemmas to be proved in terms of each other.
    [f : A → B → C] is no longer fixed to allow this flexibility. *)
Section NoDup_zip_with.
  Context {A B C : Type}.
  Implicit Types x : A.
  Implicit Types y : B.
  Implicit Types l : list A.
  ...
  Lemma NoDup_zip_with_l_strong (f : A → B → C) l k :
    (∀ i1 i2 x1 x2 y1 y2,
  ...
  Proof.
    intros Hinj.
    induction 1 as [| x l Hxl Hl IHl] in k, Hinj |- *;
      first by rewrite zip_with_nil_l.
    destruct k as [| y k]; first by rewrite zip_with_nil_r.
    simpl. constructor.
    - intros (i & x' & y' & Hf & Hx & Hy)%elem_of_lookup_zip_with_1.
      specialize Hinj with (i1:=0) (i2:=S i) (5:=Hf); simpl in Hinj.
      pose proof Hinj eq_refl eq_refl Hx Hy as <-.
      by apply elem_of_list_lookup_2 in Hx.
    - apply IHl. intros i1 i2 x1 x2 y1 y2 Hl1 Hk1 Hl2 Hk2 Hf.
      by apply Hinj with (i1:=S i1) (i2:=S i2) (5:=Hf).
  Qed.
  Lemma NoDup_zip_with_r_strong (f : B → A → C) k l :
    (∀ i1 i2 y1 y2 x1 x2,
        l !! i1 = Some x1 → k !! i1 = Some y1 →
        l !! i2 = Some x2 → k !! i2 = Some y2 →
        f y1 x1 = f y2 x2 → x1 = x2) →
    NoDup l →
    NoDup (zip_with f k l).
  Proof.
    intros Hinj. rewrite <- zip_with_flip.
    apply NoDup_zip_with_l_strong.
    intros i1 i2 x1 x2 y1 y2 Hl1 Hk1 Hl2 Hk2.
    unfold flip. eapply Hinj; eauto.
  Qed.
  Lemma NoDup_zip_with_l (f : A → B → C) `{Hf : !Inj2 (=) (=) (=) f} l k :
    NoDup l →
    NoDup (zip_with f l k).
  Proof.
    apply NoDup_zip_with_l_strong. by intros ?????? _ _ _ _ [<- _]%Hf.
  Qed.
  Lemma NoDup_zip_with_r (f : B → A → C) `{Hf : !Inj2 (=) (=) (=) f} l k :
    NoDup l →
    NoDup (zip_with f k l).
  Proof.
    apply NoDup_zip_with_r_strong. by intros ?????? _ _ _ _ [_ <-]%Hf.
  Qed.
End NoDup_zip_with.
